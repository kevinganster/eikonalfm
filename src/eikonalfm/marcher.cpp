...
MarcherInfo::MarcherInfo(int const ndim, usize const* shape)
    : ndim(ndim), shape(shape), size(std::accumulate(shape, shape + ndim, 1, std::multiplies<usize>())) {}

void SensitivityInfo::store_sequence(const usize x) { sequence[x] = counter++; }

void SensitivityInfo::store_order(const int dim, const usize x, const char o) { orders[dim * size + x] = o; }


Marcher::Marcher(double const* const c, MarcherInfo& info, double const* dx, int const order)
...
void Marcher::initialize(usize const x_s, double* const tau) {
...
void Marcher::solve(usize const x_s, double* const tau) {
    initialize(x_s, tau);
...
auto Marcher::solve_quadratic(const usize x, double* const tau) const -> double {
    usize rem = x;
    for (int d = 0; d < info.ndim; d++) {
        // index in the current axis
        usize dim_i = rem / shift[d];
