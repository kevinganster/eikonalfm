...
void SensitivityInfo::store_sequence(const usize x) { sequence[x] = counter++; }

void SensitivityInfo::store_order(const int dim, const usize x, const char o) {
  orders[dim * size + x] = o;
}

Marcher::Marcher(const double *const c, MarcherInfo &info, const double *dx,
                 const int order)
    : c(c), info(info), dx(dx), order(order) {
...
void Marcher::solve(const usize x_s, double *const tau) {
  ...
  auto heap_comp = [&tau](const usize e1, const usize e2) {
