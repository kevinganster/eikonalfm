...
template <typename _Comp> class Heap {
protected:
...
public:
  explicit Heap(const _Comp &_Comparator, const unsigned long max_size)
      : c(), comp(_Comparator),
        m(new unsigned long[max_size]) { // construct with specified comparator
  }

  ~Heap() { delete[] m; }

  bool empty() const { // test if the heap is empty
    return c.empty();
  }

  unsigned long size() const { // return length of queue
    return (unsigned long)c.size();
  }

  unsigned long top() const { return c.front(); }
  ...
  //	//for (std::map<_ItemType, unsigned long>::iterator i=m.begin(); i !=
  // m.end(); i++)
...
//	auto heap_comp = [&tau](const long e1, const long e2) { return tau[e1] <
// tau[e2]; }; 	Heap<long, decltype(heap_comp)> h(heap_comp);
//
//	h.push(0);
//	h.push(1);
//	h.push(2);
