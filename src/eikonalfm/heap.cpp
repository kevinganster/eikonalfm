...
public:
    explicit Heap(const _Comp& _Comparator, const unsigned long max_size)
        : c(), comp(_Comparator), m(new unsigned long[max_size]) { // construct with specified comparator
    }

    ~Heap() { delete[] m; }

    auto empty() const -> bool { // test if the heap is empty
        return c.empty();
    }

    auto size() const -> unsigned long { // return length of queue
        return static_cast<unsigned long>(c.size());
    }

    auto top() const -> unsigned long { return c.front(); }
...
private:
    inline auto parent(const unsigned long i) const -> unsigned long { return (i - 1) >> 1; }

    inline auto left(const unsigned long i) const -> unsigned long { return (i << 1) + 1; }

    inline auto right(const unsigned long i) const -> unsigned long { return (i << 1) + 2; }
    ...
    void sift_up(unsigned long const start) {
        unsigned long i = start;
        while (true) {
            if (i == 0)
                break;
