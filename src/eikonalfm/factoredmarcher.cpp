...
FactoredMarcher::FactoredMarcher(const double *const c, MarcherInfo &info,
                                 const double *const dx, const int order)
...
void FactoredMarcher::initialize(double *const tau0, double *const tau1,
                                 const unsigned long x_s,
                                 const long *const x_s_v) {
...
void FactoredMarcher::solve(const usize x_s, double *const tau1) {
  ...
  auto heap_comp = [&tau0, &tau1](const usize e1, const usize e2) {
    return tau(e1) < tau(e2);
  };
  ...
  while (!front.empty()) {
    ...
    // append all points that have the same (minimal) tau-value -> front.push
    // will get faster by this
    while (true) {
...
double FactoredMarcher::solve_quadratic(const double *const tau0,
                                        const double *const tau1,
                                        const ssize *const x_s, const usize x) {
  usize rem = x;
  for (int d = 0; d < info.ndim; d++) {
    // index in the current axis
    usize dim_i = rem / shift[d];
    ...
    double tau_n = INF;
    char direction = 0;
    // choose the direction considering the non-factored first order
    // approximation (tau_i-1 < tau_i+1 -> backward direction) valid known
    ...
    if (dim_i < info.shape[d] - 1 && flags[x + shift[d]] == KNOWN &&
        tau(x + shift[d]) < tau_n) {
      tau_n = tau(x + shift[d]);
      direction = 1;
    }
    ...
    if (direction != 0) {
      ...
      // valid known second neighbor + second order condition -> use second
      // order
      if (order == 2 &&
          ((direction == -1 && dim_i > 1) ||
           (direction == 1 && dim_i < info.shape[d] - 2)) &&
          flags[x + 2 * direction * shift[d]] == KNOWN &&
          tau_n > tau(x + 2 * direction * shift[d])) {
        alpha_d = 3.0 * tau0[x] / (2.0 * dx[d]) -
                  direction * (ssize)(dim_i - x_s[d]) * dx[d] / tau0[x];
        beta_d = tau0[x] *
                 (4.0 * tau1[x + direction * shift[d]] -
                  tau1[x + 2 * direction * shift[d]]) /
                 (2 * dx[d] * alpha_d);
      ...
      // otherwise fall back to first order
        ...
        alpha_d = tau0[x] / dx[d] -
                  direction * (long)(dim_i - x_s[d]) * dx[d] / tau0[x];
