...
static PyObject *fast_marching(PyObject *args, PyObject *kwargs, const bool factored) {
    ...
    if (!collection.validate(x_s_, PyExc_ValueError, "x_s must be a 1D array of nonnegative ints."))
        ...
        PyErr_SetString(
            PyExc_ValueError,
            format("size of x_s and number of dimensions of c do not match: %d != %d.", (int)PyArray_SIZE(x_s_), ndim)
                .get()
        );
        collection.DECREF();
        return NULL;
    ...
    if (output_sensitivities) {
        ...
        PyObject *sequence = PyArray_New(
            &PyArray_Type, ndim, npy_shape, NPY_LONG, NULL, info->get_sequence(), 0, NPY_ARRAY_CARRAY, NULL
        );
        if (!collection.validate(sequence, PyExc_MemoryError, "couldn't create sequence array."))
            return NULL;
        PyObject *orders = PyArray_New(
            &PyArray_Type, ndim + 1, orders_shape, NPY_INT8, NULL, info->get_order(), 0, NPY_ARRAY_CARRAY, NULL
        );
...
int main() {
    ...
    double c_[] = {
        0.50, 0.85, 1.20, 1.55, 1.90, 2.25, 2.60, 2.95, 3.30, 3.65, 4.00, 0.50, 0.85, 1.20, 1.55, 1.90, 2.25, 2.60,
        2.95, 3.30, 3.65, 4.00, 0.50, 0.85, 1.20, 1.55, 1.90, 2.25, 2.60, 2.95, 3.30, 3.65, 4.00, 0.50, 0.85, 1.20,
        1.55, 1.90, 2.25, 2.60, 2.95, 3.30, 3.65, 4.00, 0.50, 0.85, 1.20, 1.55, 1.90, 2.25, 2.60, 2.95, 3.30, 3.65,
        4.00, 0.50, 0.85, 1.20, 1.55, 1.90, 2.25, 2.60, 2.95, 3.30, 3.65, 4.00, 0.50, 0.85, 1.20, 1.55, 1.90, 2.25,
        2.60, 2.95, 3.30, 3.65, 4.00, 0.50, 0.85, 1.20, 1.55, 1.90, 2.25, 2.60, 2.95, 3.30, 3.65, 4.00, 0.50, 0.85,
        1.20, 1.55, 1.90, 2.25, 2.60, 2.95, 3.30, 3.65, 4.00, 0.50, 0.85, 1.20, 1.55, 1.90, 2.25, 2.60, 2.95, 3.30,
        3.65, 4.00, 0.50, 0.85, 1.20, 1.55, 1.90, 2.25, 2.60, 2.95, 3.30, 3.65, 4.00,
    };
    ...
    PyTuple_SET_ITEM(
        args, 0,
        PyArray_New(
            &PyArray_Type, 2, new npy_intp[2]{11, 11}, NPY_DOUBLE, NULL, (double *)&c_, 0, NPY_ARRAY_CARRAY, NULL
        )
    );
    // x_s
    PyTuple_SET_ITEM(
        args, 1,
        PyArray_New(&PyArray_Type, 1, new npy_intp[1]{2}, NPY_LONG, NULL, new long[2]{2, 0}, 0, NPY_ARRAY_CARRAY, NULL)
    );
    // dx
    PyTuple_SET_ITEM(
        args, 2,
        PyArray_New(
            &PyArray_Type, 1, new npy_intp[1]{2}, NPY_DOUBLE, NULL, new double[2]{0.4, 0.7}, 0, NPY_ARRAY_CARRAY, NULL
        )
    );
    // order
