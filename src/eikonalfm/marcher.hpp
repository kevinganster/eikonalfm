...
public:
  ...
  SensitivityInfo(const int ndim, const usize *shape)
      : MarcherInfo{ndim, shape} {
...
protected:
  ...
  // the shift to apply in every dimension to get a points neighbor (signed
  // since we sometimes multiply this with -1)
  ssize *shift;
...
private:
  // inverse squared grid-spacing in each dimension (dx^2), used in
  // solve_quadratic
  double *dx_sq_inv;
...
public:
  Marcher(const double *const c, MarcherInfo &info, const double *dx,
          const int order);
