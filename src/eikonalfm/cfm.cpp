...
public:
     ...
     */
    template<class T>
    auto validate(T* obj, PyObject* error, char const* msg) -> bool {
        if (!obj) {
        ...
        objects.push_back(reinterpret_cast<PyObject*>(obj));
        return true;
    }
...
template<class... Args>
auto format(char const* format_str, Args&&... args) -> std::unique_ptr<char[]> {
...
static auto fast_marching_(PyObject* args, PyObject* kwargs, bool const factored) -> PyObject* {
    // will be const_cast<char**>() below, hopefully PyArg_ParseTupleAndKeywords doesn't try to alter it
    char const* keywords[] = {"c", "x_s", "dx", "order", "output_sensitivities", NULL};
    // define placeholders
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "OOOi|b", const_cast<char**>(keywords), &pc, &px_s, &pdx, &order, &output_sensitivities
        ))
        return NULL;
    ...
    PyArrayObject* dx_ = reinterpret_cast<PyArrayObject*>(PyArray_FROMANY(pdx, NPY_DOUBLE, 1, 1, NPY_ARRAY_IN_ARRAY));
    if (!collection.validate(dx_, PyExc_ValueError, "dx must be a 1D array of doubles."))
        return NULL;
    else if (PyArray_SIZE(dx_) != ndim) {
        PyErr_SetString(
            PyExc_ValueError,
            format("size of dx and number of dimensions of c do not match: %d != %d.", (int)PyArray_SIZE(dx_), ndim)
                .get()
        );
        collection.DECREF();
        return NULL;
    ...
    double* dx = static_cast<double*>(PyArray_DATA(dx_));
    // TODO: maybe change this to 'ssize', since x_s_d is of type 'ssize' anyways
    usize* shape = new usize[ndim];
    // index version of x_s
    usize x_s = 0;
    npy_intp* x_s_d = static_cast<npy_intp*>(PyArray_DATA(x_s_));

    usize tmp = 1;
    for (int d = ndim - 1; d >= 0; --d) {
        shape[d] = static_cast<usize>(PyArray_DIM(c, d));

        if (x_s_d[d] < 0 || static_cast<usize>(x_s_d[d]) >= shape[d]) {
            PyErr_SetString(
                PyExc_ValueError,
                format(
                    "entries of x_s have to be within the shape of c, but entry %d with value %ld is not in [0, %ld].",
                    d, x_s_d[d], shape[d] - 1
                )
                    .get()
            );
        ...
        if (dx[d] <= 0) {
            PyErr_SetString(
                PyExc_ValueError,
                format("entries of dx must be greater than zero, but entry %d with value %f is not.", d, dx[d]).get()
            );
            collection.DECREF();
            return NULL;
        }

        x_s += tmp * static_cast<usize>(x_s_d[d]);
        tmp *= shape[d];
    }

    // create a new array for the return value
    PyArrayObject* tau = reinterpret_cast<PyArrayObject*>(PyArray_ZEROS(ndim, PyArray_DIMS(c), NPY_DOUBLE, 0));
    ...
    Marcher* m;
    if (factored)
        m = new FactoredMarcher{static_cast<double*>(PyArray_DATA(c)), *info, dx, order};
    else
        m = new Marcher{static_cast<double*>(PyArray_DATA(c)), *info, dx, order};
    ...
    Py_BEGIN_ALLOW_THREADS // release GIL
        try {
        m->solve(x_s, static_cast<double*>(PyArray_DATA(tau)));
    } catch (const InterruptExc& ex) {
        error_type = PyExc_KeyboardInterrupt;
        success = false;
    ...
    if (!success) {
        PyErr_SetString(error_type, error_msg.c_str());
        if (output_sensitivities) {
    ...
    if (output_sensitivities) {
        ...
        PyObject* sequence = PyArray_SimpleNewFromData(ndim, npy_shape, NPY_ULONG, info->get_sequence());
        ...
        PyArray_ENABLEFLAGS(reinterpret_cast<PyArrayObject*>(sequence), NPY_ARRAY_OWNDATA);
        ...
        PyArray_ENABLEFLAGS(reinterpret_cast<PyArrayObject*>(orders), NPY_ARRAY_OWNDATA);
        ...
        res = reinterpret_cast<PyObject*>(tau);
    }
...
static auto fast_marching_wrapper(PyObject* self, PyObject* args, PyObject* kwargs) -> PyObject* {
    return fast_marching_(args, kwargs, false);
}

static auto factored_marching_wrapper(PyObject* self, PyObject* args, PyObject* kwargs) -> PyObject* {
...
static PyMethodDef fm_methods[] = {
    {"fast_marching", (PyCFunction)fast_marching_wrapper, METH_VARARGS | METH_KEYWORDS,
     ...
     "        Source position as index vector, e.g. ``(0, 0)``. Must have the same length as the number of dimensions "
     "of c.\n"
     ...
     "        Only returned when 'output_sensitivities=True'. The finite difference orders for each dimension used at "
     "each gridpoint."},
