...
#include <stdexcept>
#include <vector>
// #include "numpy/noprefix.h" includes "no-prefix option" of the types and
// methods, i.e. intp instead of npy_intp defining NPY_NO_PREFIX prior to
// including "numpy/arrayobject.h" does this as well
...
void signal_handler(int code) { throw InterruptExc(); };
...
public:
   ...
   * If it fails, a specified error will be returned to Python and all stored
   * pointers will be dereferenced. Otherwise it adds the pointer to the list of
   * stored pointers.
   */
  template <class T> bool validate(T *obj, PyObject *error, const char *msg) {
    if (!obj) {
...
static PyObject *fast_marching_(PyObject *args, PyObject *kwargs,
                                const bool factored) {
  // will be const_cast<char**>() below, hopefully PyArg_ParseTupleAndKeywords
  // doesn't try to alter it
  const char *keywords[] = {"c", "x_s", "dx", "order", "output_sensitivities",
                            NULL};
  // define placeholders
  ...
  // parse arguments
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOOi|b",
                                   const_cast<char **>(keywords), &pc, &px_s,
                                   &pdx, &order, &output_sensitivities))
    return NULL;
  ...
  PyArrayObject *c = (PyArrayObject *)PyArray_FROMANY(pc, NPY_DOUBLE, 1, 0,
                                                      NPY_ARRAY_IN_ARRAY);
  if (!collection.validate(c, PyExc_ValueError,
                           "c must be an array of doubles."))
    return NULL;

  int ndim = PyArray_NDIM(c);

  // check if px0 is a python-numpy-array and if it has the correct size
  PyArrayObject *x_s_ = (PyArrayObject *)PyArray_FROMANY(px_s, NPY_LONG, 1, 1,
                                                         NPY_ARRAY_IN_ARRAY);
  if (!collection.validate(x_s_, PyExc_ValueError,
                           "x_s must be a 1D array of nonnegative ints."))
    return NULL;
  else if (PyArray_SIZE(x_s_) != ndim) {
    PyErr_SetString(
        PyExc_ValueError,
        format(
            "size of x_s and number of dimensions of c do not match: %d != %d.",
            (int)PyArray_SIZE(x_s_), ndim)
            .get());
    collection.DECREF();
    return NULL;
  }
  ...
  PyArrayObject *dx_ = (PyArrayObject *)PyArray_FROMANY(pdx, NPY_DOUBLE, 1, 1,
                                                        NPY_ARRAY_IN_ARRAY);
  if (!collection.validate(dx_, PyExc_ValueError,
                           "dx must be a 1D array of doubles."))
    return NULL;
  else if (PyArray_SIZE(dx_) != ndim) {
    PyErr_SetString(
        PyExc_ValueError,
        format(
            "size of dx and number of dimensions of c do not match: %d != %d.",
            (int)PyArray_SIZE(dx_), ndim)
            .get());
    collection.DECREF();
    return NULL;
  }
  ...
  for (int d = ndim - 1; d >= 0; d--) {
    ...
    if (x_s_d[d] >= shape[d]) {
      PyErr_SetString(PyExc_ValueError,
                      format("entries of x_s have to be within the shape of c, "
                             "but entry %d with value %ld is not in [0, %ld].",
                             d, x_s_d[d], shape[d] - 1)
                          .get());
      collection.DECREF();
      return NULL;
    ...
    if (dx[d] <= 0) {
      PyErr_SetString(PyExc_ValueError,
                      format("entries of dx must be greater than zero, but "
                             "entry %d with value %f is not.",
                             d, dx[d])
                          .get());
      collection.DECREF();
      return NULL;
  ...
  PyArrayObject *tau =
      (PyArrayObject *)PyArray_ZEROS(ndim, PyArray_DIMS(c), NPY_DOUBLE, 0);
  if (!collection.validate(tau, PyExc_MemoryError,
                           "couldn't create result array."))
    return NULL;
  ...
  if (output_sensitivities) {
    ...
    // the pointers of `info` must not be deallocated! otherwise the numpy array
    // is broken
    PyObject *sequence = PyArray_SimpleNewFromData(ndim, npy_shape, NPY_LONG,
                                                   info->get_sequence());
    if (!collection.validate(sequence, PyExc_MemoryError,
                             "couldn't create sequence array.")) {
      delete[] info->get_sequence();
      delete[] info->get_order();
      return NULL;
    }
    // set the array object to own the data pointer and therefore call delete[]
    // when it is destroyed
    PyArray_ENABLEFLAGS((PyArrayObject *)sequence, NPY_ARRAY_OWNDATA);

    PyObject *orders = PyArray_SimpleNewFromData(ndim + 1, orders_shape,
                                                 NPY_INT8, info->get_order());
    if (!collection.validate(orders, PyExc_MemoryError,
                             "couldn't create sequence array.")) {
      // info->get_sequence is already owned by the array, so don't delete it
      // here
      delete[] info->get_order();
      return NULL;
    }
    ...
    Py_INCREF(tau); // need to increase by 1 here so that after
                    // collection.DECREF the count is still 1
    res = (PyObject *)tau;
  }
...
static PyMethodDef fm_methods[] = {
    {"fast_marching", (PyCFunction)fast_marching_wrapper,
     ...
     "        Source position as index vector, e.g. ``(0, 0)``. Must have the "
     "same length as the number of dimensions of c.\n"
     "    dx : sequence of doubles\n"
     "        Grid spacing for each dimension, dx > 0. Must have the same "
     "length as the number of dimensions of c.\n"
     "    order : {1, 2}\n"
     ...
     "        Only returned when 'output_sensitivities=True'. The sequence in "
     "which each gridpoint was set to known.\n"
     "    orders : ndarray, optional\n"
     "        Only returned when 'output_sensitivities=True'. The finite "
     "difference orders for each dimension used at each gridpoint."},
    {"factored_fast_marching", (PyCFunction)factored_marching_wrapper,
     METH_VARARGS | METH_KEYWORDS,
     "factored_fast_marching(c, x_s, dx, order, "
     "output_sensitivities=False)\n--\n\n"
     "    Calculates the fast marching solution to the factored eikonal "
     "equation.\n"
     "\n"
     "    Parameters\n"
     "    ----------\n"
     ...
     "        To get tau, you need to multiply this with the distance field "
     "tau0.\n"
     "    sequence : ndarray, optional\n"
     "        Only returned when 'output_sensitivities=True'. The sequence in "
     "which each gridpoint was set to known.\n"
...
static struct PyModuleDef cfm_module = {
    ...
    "c++ implementation of (factored) fast marching for the eikonal "
    "equation", // Module description
    0, // size of per-interpreter state of the module, or -1 if the module
