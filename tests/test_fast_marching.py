...
def test_fm():
    ...
    tau, sequence, orders = eikonalfm.fast_marching(
        c, x_s, dx, 2, output_sensitivities=True
    )
    assert isinstance(tau, np.ndarray) and tau.dtype == float and tau.shape == c.shape
    assert (
        isinstance(sequence, np.ndarray)
        and sequence.dtype == int
        and sequence.shape == c.shape
    )
    assert (
        isinstance(orders, np.ndarray)
        and orders.dtype == np.int8
        and orders.shape == (c.ndim, *c.shape)
    )
...
def test_ffm():
    ...
    assert (
        isinstance(tau1, np.ndarray) and tau1.dtype == float and tau1.shape == c.shape
    )

    tau1, sequence, orders = eikonalfm.fast_marching(
        c, x_s, dx, 2, output_sensitivities=True
    )
    assert (
        isinstance(tau1, np.ndarray) and tau1.dtype == float and tau1.shape == c.shape
